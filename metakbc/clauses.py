...
def load_clauses(dataset: Dataset) -> List[LearnedClause]:
    ...
    if dataset.name in ["Toy_mixed"]:
        ...
        clause2 = LearnedClause(n_variables=2, n_relations=2, clause_loss_func=clause_func2, n_constraints=10, n_predicates=dataset.n_predicates)

        # A(X1, X2), B(X2, X3) => C(X1, X3)
        clause_func3 = lambda x1, x2, x3, phi1, phi2, phi3: torch.min(phi1(x1, x2), phi2(x2, x3)) - phi3(x1, x3)
        clause3 = LearnedClause(n_variables=3, n_relations=3, clause_loss_func=clause_func3, n_constraints=10, n_predicates=dataset.n_predicates)
