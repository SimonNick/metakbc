...
```bash
$ ls *data=umls*.log | xargs grep "dev res" | awk '{ print $6 " " $1 }' | tr ":" " " | grep -v H@1 | sort -n | awk '{ print $1 " " $2 }' | tail -n 1
...
```bash
$ ls *data=nations*.log | xargs grep "dev res" | awk '{ print $6 " " $1 }' | tr ":" " " | grep -v H@1 | sort -n | awk '{ print $1 " " $2 }' | tail -n 1
...
```bash
$ ls *data=kinship*.log | xargs grep "dev res" | awk '{ print $6 " " $1 }' | tr ":" " " |  grep -v H@1 | sort -n | awk '{ print $1 " " $2 }' | tail -n 1
...
0.369825 fb15k-237_beaker_v1.V=3_b=500_e=100_f2=0_i=standard_k=2000_lr=0.1_m=complex_n3=0.1_o=adagrad.log

$ cat fb15k-237_beaker_v1.V=3_b=500_e=100_f2=0_i=standard_k=2000_lr=0.1_m=complex_n3=0.1_o=adagrad.log | grep 369825 -A 1
INFO:meta-cli.py:Epoch 99/100	dev results	MRR 0.369825	H@1 0.276333	H@3 0.406302	H@5 0.471286	H@10 0.558540
...
```bash
$ ls *.log | xargs grep "dev res" |  awk '{ print $6 " " $1 }' | tr ":" " " |  grep -v H@1 | sort -n | awk '{ print $1 " " $2 }' | tail -n 1
0.488854 wn18rr_beaker_v1.V=3_b=100_e=100_f2=0_i=reciprocal_k=4000_lr=0.1_m=complex_n3=0.1_o=adagrad.log
